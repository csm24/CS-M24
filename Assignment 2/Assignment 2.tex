...
If network is selected, we will be playing another device - but is that device using a chess engine or a human player? To resolve these issues, we will split the players into \textbf{Me} and \textbf{Other}, with options for each as in this table:
...
\begin{tabular}{| l | p{10cm}  |  c  | c |} \hline  
...
\textbf{REQ-P1:}  & The application must store at least 8 player profiles.  & Spec & \textit{Priority : High} \\

\textbf{REQ-P2:} & The profiles must be persisted without any user interaction and reloaded when the application starts.  & spec & \textit{Priority : High}\\
...
& Derived & \textit{Priority : High}\\

\textbf{REQ-P4:} &  The menu that allows a user to select the type of player must have quick access to player profiles
& Spec  &  \textit{Priority : Medium}\\

\textbf{REQ-P5:} 
&  To start a game, the user may select from three types of player, \textit{Computer, Human. Network}
& Spec  &  \textit{Priority : High}\\
\textbf{REQ-P6:} 
&  To start a game, the user may select from three types of player, \textit{Computer, Human. Network}
& Spec  &  \textit{Priority : High}\\
\textbf{REQ-P7:} 
...
This section describes the requirements relating to the user profiles.
